...
var upgTool = (function(window, document, undefined) {
  ...
  upgTool.hideLoading = function($container){
	...
	upgTool.ajax = function(options){
		...
		var beforeSend = options['beforeSend'] || function(){if($submitBtn.length){upgTool.disableButton($submitBtn); } };
		var complete = options['complete'] || function(){if($submitBtn.length){upgTool.enableButton($this); } };
	...
	upgTool.percent = function(){
		$("[percent]").each(function(){
			var $this = $(this);
			var num = parseInt($this.attr('percent'));
			var x, y;
			if(!isNaN(num)){
				x = -47 * num;
				// 设置位置
  ...
  upgTool.loadAjaxData = function(url, selector, gotop, callback) {
      ...
      $(selector).load(url, {
          ...
          if (gotop) {
              $(window).scrollTop(typeof(gotop) == 'number' ? gotop : 0);
          }
          callback();
      });
  };
  ...
  upgTool.eventBroker = {
    ...
    eventExec: function(ev, origin, eventGroup){
      var e = upgTool.eventBroker.eventAnalysis(origin);
    ...
    throwEvent: function(ev, eventGroup){
      ...
      upgTool.eventBroker.eventExec.call(this, ev, {event: _fn, args: _args}, eventGroup);
    },
